...
const courseList = async (req, res) => {
    ...
    await connection.query(`SELECT * FROM
                            (SELECT class_id, 'teach' class_type, semester,year,courseno,seclec,seclab,ip_type,yearly FROM tbl_class WHERE courseno IN (:courseno)
                            UNION
                            SELECT class_id, 'inList' class_type, semester,year,courseno,seclec,seclab,ip_type,yearly FROM tbl_class WHERE instructor_id IN(:instructorId)) main
                            JOIN
